...
import { analyzeGoalPotential, type RiskLevel } from './goalPotential.js';
...
let cache: { at: number; data: LiveHeatBoard } | null = null;
...
function isEligible(m: LiveFeedMatch): boolean {
  if (m.status === 'FT' || m.status === 'NS') return false;
  if (m.status === 'HT' || m.status === 'ET') return true;
...
function toHeatPick(match: LiveFeedMatch, stats: FeedStats | null): LiveHeatPick {
  ...
  const heatCorners =
    Math.min(cornersTotal / 14, 1) * 0.45 +
    pCorner * 0.4 +
    analysis.model.cornerIntensity * 0.15;
  const heatShots =
    Math.min(shotsOnTotal / 12, 1) * 0.4 +
    pGoal * 0.45 +
...
export async function scanLiveHeat(opts?: { force?: boolean }): Promise<LiveHeatBoard> {
  const now = Date.now();
