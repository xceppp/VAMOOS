...
const AISCORE_RE =
  /^https?:\/\/(?:www\.|m\.)?aiscore\.com\/(?:[a-z]{2}\/)?match-([^/?#]+)\/([a-z0-9]+)\/?/i;

export function isAiscoreUrl(url: string): boolean {
  try {
...
export function parseAiscoreUrl(url: string): Pick<AiscoreParsed, 'url' | 'matchId' | 'slug' | 'home' | 'away' | 'notes'> {
...
export function parseAiscorePaste(text: string, base?: Partial<AiscoreParsed>): AiscoreParsed {
  ...
  let minute: number | null = base?.minute ?? null;
  const minM =
    t.match(/\b(\d{1,3})['′’]/) ||
    t.match(/\b(\d{1,3})\s*min(?:ute)?s?\b/i);
  if (minM) minute = Math.min(120, Number(minM[1]));
  if (ht && minute == null) {
    minute = 45;
    notes.push('Only HT found — assumed minute ~45 unless you set it.');
  }

  let status: string | null = base?.status ?? null;
  if (/full\s*time|\bFT\b/i.test(t)) status = 'FT';
  else if (minute != null && minute !== 45) status = 'LIVE';
  else if (/\bHT\b|half\s*time/i.test(t) && (minute == null || minute === 45)) status = 'HT';
  ...
  const pair = (label: string): [number | null, number | null] => {
  ...
  let attacksHome: number | null = null;
  let attacksAway: number | null = null;
