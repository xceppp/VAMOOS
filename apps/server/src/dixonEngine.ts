...
interface FittedLeague {
  slug: string;
  name: string;
  avgHome: number;
  avgAway: number;
  teams: Map<number, FittedTeam>;
  byNorm: Map<string, FittedTeam>;
  history: Array<{ homeId: number; awayId: number; hg: number; ag: number }>;
  /** True when pack shipped real finished rows (not synthesized priors). */
  realHistory: boolean;
  cal: {
    rho: number;
    homeAdv: number;
...
const fittedCache = new Map<string, FittedLeague>();
...
function synthesizeHistory(
  ...
  if (teams.length < 2) return history;
  const now = Date.now();
...
function movEloMultiplier(goalDiff: number): number {
...
function fitLeague(slug: string, pack: LeaguePack): FittedLeague {
  if (fittedCache.has(slug)) return fittedCache.get(slug)!;
  ...
  const samples = new Map<number, Array<{ gf: number; ga: number; isHome: boolean; w: number }>>();
  const elo = new Map<number, number>();
  const names = new Map<number, string>();
  const historyPairs: FittedLeague['history'] = [];
  ...
  for (let idx = 0; idx < sorted.length; idx++) {
    ...
    homeW.push(w);
    awayW.push(w);
    if (!samples.has(hid)) samples.set(hid, []);
    if (!samples.has(aid)) samples.set(aid, []);
  ...
  const shrink = (obs: number, nEff: number, prior = 1) =>
    (nEff * obs + priorStrength * prior) / (nEff + priorStrength);

  const teams = new Map<number, FittedTeam>();
  const byNorm = new Map<string, FittedTeam>();

  const allIds = new Set<number>([...names.keys(), ...packTeams.map((t) => t.id)]);
  ...
  for (const tid of allIds) {
    ...
    let attackHome: number;
    let defenseHome: number;
    let attackAway: number;
    let defenseAway: number;
    ...
    if (packT) {
      ...
      if (packT.elo != null) teamElo = 0.55 * teamElo + 0.45 * packT.elo;
    }
...
function canonicalNorm(name: string): string {
  const n = normName(name);
  return TEAM_ALIASES[n] || n;
}

function findFittedTeam(name: string, league: FittedLeague): FittedTeam | null {
  const n = canonicalNorm(name);
  if (!n) return null;
  for (const [tn, team] of league.byNorm) {
    if (canonicalNorm(tn) === n || tn === n) return team;
  }
  let best: FittedTeam | null = null;
  let bestLen = 0;
  for (const [tn, team] of league.byNorm) {
...
function formIndex(team: FittedTeam): number {
  if (!team.form.length) return 0.5;
...
function h2hStats(
  ...
  homeId: number,
  awayId: number,
): { meetings: number; avgHome: number; avgAway: number } {
  let meetings = 0;
  let sumH = 0;
  let sumA = 0;
  ...
  if (!meetings) return { meetings: 0, avgHome: 0, avgAway: 0 };
...
function expectedGoals(
  ...
  formHome: number;
  formAway: number;
  h2h: ReturnType<typeof h2hStats>;
} {
 ...
 */
function pickMostLikelyScore(
  grid: number[][],
  pHome: number,
  pDraw: number,
  pAway: number,
): { best: string; top: Array<{ score: string; prob: number }> } {
  type Cell = { i: number; j: number; p: number };
  const cells: Cell[] = [];
  for (let i = 0; i < grid.length; i++) {
    for (let j = 0; j < (grid[i]?.length || 0); j++) {
      const p = grid[i][j] || 0;
      if (p > 0) cells.push({ i, j, p });
    }
  }
  ...
  if (pHome >= pDraw && pHome >= pAway) side = 'home';
  else if (pAway >= pDraw && pAway >= pHome) side = 'away';

  const matchesSide = (c: Cell) => {
    if (side === 'home') return c.i > c.j;
    if (side === 'away') return c.i < c.j;
    return c.i === c.j;
  };
...
function buildMarkets(args: {
  ...
  if (isLive && scored) {
    const live = liveResidual(
      args.lamH,
      args.lamA,
      scored[0],
      scored[1],
      remainingFraction(args.minute, args.status),
    );
    pHome = live.p_home;
    pDraw = live.p_draw;
    pAway = live.p_away;
...
function predictOne(m: MatchInput, nextSynth: { n: number }): DixonEnginePick | { skip: string } {
  ...
  if (isLive && scored) {
    scoreGrid = liveFinalMatrix(
      lamH,
      lamA,
      scored[0],
      scored[1],
      remainingFraction(m.minute, m.status),
    );
  }
...
export function clearDixonCaches(): void {
  packCache.clear();
  fittedCache.clear();
}
