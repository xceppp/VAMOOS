import { isAiscoreUrl, parseAiscorePaste, parseAiscoreUrl, } from './aiscoreParse.js';
import { analyzeGoalPotential } from './goalPotential.js';
export async function tryFetchAiscore(url) {
    try {
        ...
        if (/just a moment|cloudflare|cf-chl|challenge-platform/i.test(text))
            return null;
        if (text.length < 2000)
            return null;
        return text;
...
export async function buildAiscoreAnalysis(body) {
    ...
    if (url) {
        ...
        if (html) {
            ...
            parsed.notes.unshift('AiScore blocked automatic fetch (Cloudflare). Paste match text or fill stats below.');
        }
    }
    ...
    const hasLiveStats = parsed.shotsOnHome != null ||
    ...
    if (!hasScore || (!hasLiveStats && parsed.minute == null)) {
        parsed.notes.push('Need score + minute and/or live stats (shots / dangerous attacks) for a real Bet or Nah. Paste the AiScore page or fill the manual form.');
    }
