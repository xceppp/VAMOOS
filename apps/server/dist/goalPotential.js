...
function riskFromProb(p, greenAt, orangeAt) {
    if (p >= greenAt)
        return 'green';
    if (p >= orangeAt)
        return 'orange';
...
function callFromRisk(risk, leanOrange = true) {
    if (risk === 'green')
        return 'BET';
    if (risk === 'orange')
        return leanOrange ? 'LEAN BET' : 'LEAN NAH';
    return 'NAH';
}
function betterRisk(a, b) {
    const rank = { green: 2, orange: 1, red: 0 };
    return rank[a] >= rank[b] ? a : b;
}
export function analyzeGoalPotential(input) {
    ...
    if (!isFt) {
        if (minute >= 90)
            effectiveLeft = Math.max(effectiveLeft, 4);
        else if (minute >= 85)
            effectiveLeft = Math.max(minutesLeft, 5);
        else if (minute >= 75)
            effectiveLeft = minutesLeft + 2.5;
        else if (input.status === 'ET')
            effectiveLeft = Math.max(minutesLeft, 8);
    }
    ...
    const attacks = Math.max(n(input.attacksHome) + n(input.attacksAway), shotsOn + shotsOff);
    ...
    const pressureRate = (dangerous / elapsed) * 0.012 + (attacks / elapsed) * 0.0035 + (corners / elapsed) * 0.01;
    ...
    if (isDraw && minute >= 70)
        stateMult *= 1.18;
    if (scoreGap >= 3)
        stateMult *= 0.72; // dead rubber
    if (scoreGap === 2 && minute >= 80)
        stateMult *= 0.9;
    // One team dominating possession but match still open
    if (Math.abs(possH - 50) >= 18 && isClose)
        stateMult *= 1.08;
    // Sterile domination: high poss, tiny shot volume
    if (possH >= 65 && shotsOn <= 2 && minute >= 60)
        stateMult *= 0.8;
    // Late window: games stretch, benches push
    let lateMult = 1;
    if (minute >= 75 && isClose)
        lateMult *= 1.28;
    else if (minute >= 75)
        lateMult *= 1.12;
    if (minute >= 85 && isClose)
        lateMult *= 1.15;
    if (minute >= 88)
        lateMult *= 1.1;
    ...
    const intensity = clamp((sotPer10 / 1.4) * 0.34 +
    ...
    const blendedCornerPace = elapsed < 25 ? cornerPace * 0.55 + priorCornerPace * 0.45 : cornerPace * 0.8 + priorCornerPace * 0.2;
    // Corners track attacks / shots / one-way traffic
    const attackCornerSignal = (attacks / elapsed) * 0.004 + (shotsOn / elapsed) * 0.015 + (dangerous / elapsed) * 0.006;
    let cornerRate = blendedCornerPace * 0.7 + attackCornerSignal * 0.3;
    let cornerState = 1;
    if (isClose && minute >= 70)
        cornerState *= 1.2;
    if (isDraw && minute >= 80)
        cornerState *= 1.15;
    if (scoreGap >= 3)
    ...
    const cornerIntensity = clamp((cornerPace / 0.14) * 0.45 + (attacks / elapsed / 0.8) * 0.25 + pNextCorner * 0.3, 0, 1);
    // ---- Verdicts ----
    const reasons = [];
    ...
    reasons.push(`P(goal) ${(pNextGoal * 100).toFixed(0)}% (λ=${expectedExtraGoals.toFixed(2)}) · P(corner) ${(pNextCorner * 100).toFixed(0)}% (λ=${expectedExtraCorners.toFixed(2)})`);
    let goalRisk = 'red';
    let call = 'NAH';
