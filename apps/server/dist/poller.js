...
function sideStatsEqual(a, b) {
    if (a === b)
        return true;
    if (!a || !b)
        return !a && !b;
    return (a.possessionHome === b.possessionHome &&
...
function carryBoardExtras(prevMatch, next) {
    if (!prevMatch)
        return next;
    return {
        ...next,
...
export function startPoller(hub, options) {
    ...
    function snapshotPayload() {
        return {
            ...
            mode: state.mode === 'cached' ? 'live' : state.mode,
    ...
    function commitMatches(next, emitDiff) {
        ...
        try {
            ...
            await mapPool(targets, 6, async (m) => {
                ...
                const nextOdds = odds
                    ...
                    : m.odds;
                if (sideStatsEqual(m.stats, nextStats) && oddsEqual(m.odds, nextOdds))
                    return;
                updated.set(m.id, {
                    ...m,
                    stats: nextStats,
